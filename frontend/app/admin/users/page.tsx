...
export default function AdminUsersPage() {
  ...
  const toggleBan = async (user: UserRow) => {
    ...
    try {
      await api.patch(`/admin/users/${user._id}`, { banned: !user.banned });
      setUsers(prev => prev.map(u => u._id === user._id ? { ...u, banned: !u.banned } : u));
      toast.success(user.banned ? `${user.name} unbanned` : `${user.name} banned from platform`);
    } catch { toast.error("Action failed"); }
    finally { setActionId(null); }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            {users.length} registered · {counts.banned} banned
          </p>
        </div>
        <button onClick={load}
          className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium text-slate-600 hover:text-deep transition-colors"
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 rounded-xl text-sm bg-white text-deep placeholder:text-slate-400 focus:outline-none transition-all"
          style={{ border: "1.5px solid rgba(255,45,120,0.15)", boxShadow: "none" }}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white text-sm font-bold flex-shrink-0"
                            style={{ background: user.banned ? "#94A3B8" : "linear-gradient(135deg,#FF2D78,#FF6B9D)" }}>
                            {user.name?.[0]?.toUpperCase() ?? "?"}
                          </div>
                          <div className="min-w-0">
                            <div className="font-semibold text-sm text-deep truncate flex items-center gap-1.5">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            title={user.role === "admin" ? "Cannot ban admins" : ""}
                          >
                            {busy ? (
                          ...
                          <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                            style={{ background: "rgba(239,68,68,0.06)", color: "#EF4444", border: "1px solid rgba(239,68,68,0.12)" }}
                            title={user.role === "admin" ? "Cannot delete admins" : ""}
                          >
                            <Trash2 size={11} />
                          </button>
                        </div>
        ...
        {confirmDelete && (
          ...
          >
            ...
            >
              ...
              <h3 className="font-black text-lg text-deep text-center mb-1">Delete User?</h3>
              <p className="text-slate-500 text-sm text-center mb-6">
                <strong>{confirmDelete.name}</strong> ({confirmDelete.email}) will be permanently removed. This cannot be undone.
              </p>
              <div className="flex gap-3">
                <button onClick={() => setConfirmDelete(null)}
                  className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-slate-600 transition-colors"
                  style={{ background: "#F1F5F9", border: "1px solid #E2E8F0" }}>
                  Cancel
                </button>
                <button onClick={() => deleteUser(confirmDelete)}

    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-5" style={{ borderBottom: "1px solid #F1F5F9" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-black text-sm text-deep leading-none">Admin Panel</div>
            <div className="text-xs text-slate-400 mt-0.5">PhotoFly</div>
          </div>
        </div>
      </div>
...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  // current page title
  const pageTitle = navItems.find(n => n.href === pathname || (n.href !== "/admin" && pathname.startsWith(n.href)))?.label ?? "Admin";
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: "#F8FAFC" }}>
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              transition={{ type: "spring", damping: 26, stiffness: 280 }}
              className="fixed left-0 top-0 bottom-0 w-60 z-50 lg:hidden bg-white"
              style={{ borderRight: "1px solid #F1F5F9", boxShadow: "4px 0 24px rgba(0,0,0,0.08)" }}>
              <div className="absolute top-4 right-4">
                <button onClick={() => setSidebarOpen(false)}
                  className="p-1.5 rounded-xl text-slate-400 hover:text-deep hover:bg-slate-100 transition-colors">
                  <X size={16} />
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top bar */}
        <header className="px-5 py-3.5 flex items-center gap-4 bg-white flex-shrink-0"
          style={{ borderBottom: "1px solid #F1F5F9", boxShadow: "0 1px 6px rgba(0,0,0,0.03)" }}>
          <button className="lg:hidden p-2 rounded-xl text-slate-500 hover:text-deep hover:bg-slate-100 transition-colors"
            onClick={() => setSidebarOpen(true)}>
            <Menu size={18} />
          </button>
